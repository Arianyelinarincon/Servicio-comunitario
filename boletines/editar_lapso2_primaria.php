...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $analisis = trim($_POST['m2_formacion'] ?? '');
    $sugerencias = trim($_POST['m2_sugerencias'] ?? '');
    $sql = "UPDATE boletines SET m2_proyecto=?, m2_formacion=?, m2_sugerencias=? WHERE id=? AND tipo_boletin='primaria'";
    $stmt = $conexion->prepare($sql);
    $stmt->bind_param('sssi', $proyecto, $analisis, $sugerencias, $boletin_id);
    if ($stmt->execute()) {
        $_SESSION['l2_proyecto'] = $proyecto;
        $_SESSION['l2_analisis'] = $analisis;
        $_SESSION['l2_sugerencias'] = $sugerencias;
        $_SESSION['mensaje_exito'] = 'Lapso 2 actualizado correctamente.';
    } else {
...
$stmt = $conexion->prepare("SELECT m2_proyecto, m2_formacion, m2_sugerencias FROM boletines WHERE id = ?");
...
<head>
    <meta charset="UTF-8">
    <title>Editar Lapso 2 - Primaria</title>
...
<div class="container mt-5">
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h4><i class="fas fa-edit"></i> Editar Lapso 2 - Primaria</h4>
        </div>
        <div class="card-body">
            <form method="POST">
                <div class="mb-3">
                    <label for="m2_proyecto" class="form-label">Proyectos de Aprendizaje</label>
                    <textarea class="form-control" id="m2_proyecto" name="m2_proyecto" rows="3"><?= htmlspecialchars($datos['m2_proyecto'] ?? '') ?></textarea>
                </div>
                <div class="mb-3">
                    <label for="m2_formacion" class="form-label">Análisis Cualitativo</label>
