...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $analisis = trim($_POST['m1_formacion'] ?? '');
    $sugerencias = trim($_POST['m1_sugerencias'] ?? '');
    $sql = "UPDATE boletines SET m1_proyecto=?, m1_formacion=?, m1_sugerencias=? WHERE id=? AND tipo_boletin='primaria'";
    $stmt = $conexion->prepare($sql);
    $stmt->bind_param('sssi', $proyecto, $analisis, $sugerencias, $boletin_id);
    if ($stmt->execute()) {
        $_SESSION['l1_proyecto'] = $proyecto;
        $_SESSION['l1_analisis'] = $analisis;
        $_SESSION['l1_sugerencias'] = $sugerencias;
        $_SESSION['mensaje_exito'] = 'Lapso 1 actualizado correctamente.';
    } else {
...
$stmt = $conexion->prepare("SELECT m1_proyecto, m1_formacion, m1_sugerencias FROM boletines WHERE id = ?");
...
<head>
    <meta charset="UTF-8">
    <title>Editar Lapso 1 - Primaria</title>
...
<div class="container mt-5">
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h4><i class="fas fa-edit"></i> Editar Lapso 1 - Primaria</h4>
        </div>
        <div class="card-body">
            <form method="POST">
                <div class="mb-3">
                    <label for="m1_proyecto" class="form-label">Proyectos de Aprendizaje</label>
                    <textarea class="form-control" id="m1_proyecto" name="m1_proyecto" rows="3"><?= htmlspecialchars($datos['m1_proyecto'] ?? '') ?></textarea>
                </div>
                <div class="mb-3">
                    <label for="m1_formacion" class="form-label">Análisis Cualitativo</label>
