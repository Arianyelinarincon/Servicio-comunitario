...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Guardar en sesión
    $_SESSION['l1_proyecto'] = htmlspecialchars($_POST['l1_proyecto']);
    $_SESSION['l1_analisis'] = htmlspecialchars($_POST['l1_analisis']);
    $_SESSION['l1_sugerencias'] = htmlspecialchars($_POST['l1_sugerencias']);
    ...
    if ($existe) {
        ...
        $stmt->bind_param("sssiss", 
            $_SESSION['l1_proyecto'], 
            $_SESSION['l1_analisis'],
            $_SESSION['l1_sugerencias'],
...
<div style="font-family: 'Segoe UI', Arial, sans-serif; background: #f0f2f5; padding: 20px;">
    <div style="background: white; padding: 30px; border-radius: 10px; max-width: 900px; margin: 0 auto; box-shadow: 0 4px 20px rgba(0,0,0,0.08);">
        <h2 style="color: #1a237e; text-align: center;">Editar Primer Lapso - Primaria</h2>
        ...
        <form method="POST">
            <div style="border-left: 4px solid #1a237e; padding-left: 15px; margin-bottom: 20px;">
                <p style="font-size: 13px; color: #555; margin: 0;">
                    <i class="fas fa-info-circle" style="color: #1a237e;"></i> 
                    Complete los campos para el <strong>Primer Lapso</strong> de evaluación.
                </p>
            </div>
            
            <p style="font-weight: bold; font-size: 14px; margin-top: 20px;">Proyecto de Aprendizaje:</p>
            <textarea name="l1_proyecto" rows="2" required style="width: 100%; padding: 10px; border: 2px solid #dee2e6; border-radius: 6px; box-sizing: border-box; font-size: 14px;"><?php echo htmlspecialchars($_SESSION['l1_proyecto'] ?? ''); ?></textarea>
            
            <p style="font-weight: bold; font-size: 14px; margin-top: 20px;">Análisis Cualitativo:</p>
            <textarea name="l1_analisis" rows="5" required style="width: 100%; padding: 10px; border: 2px solid #dee2e6; border-radius: 6px; box-sizing: border-box; font-size: 14px;"><?php echo htmlspecialchars($_SESSION['l1_analisis'] ?? ''); ?></textarea>
            
            <p style="font-weight: bold; font-size: 14px; margin-top: 20px;">Sugerencias:</p>
            <textarea name="l1_sugerencias" rows="3" required style="width: 100%; padding: 10px; border: 2px solid #dee2e6; border-radius: 6px; box-sizing: border-box; font-size: 14px;"><?php echo htmlspecialchars($_SESSION['l1_sugerencias'] ?? ''); ?></textarea>
