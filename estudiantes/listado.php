...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    ...
    $orden = isset($_GET['orden']) ? trim($_GET['orden']) : 'reciente';
    ...
    $sql = "
        SELECT DISTINCT e.id, e.nombre, e.apellido, e.cedula_escolar, e.sala, e.genero,
               ...
               (SELECT ano_escolar FROM inscripciones WHERE estudiante_id = e.id ORDER BY fecha_inscripcion DESC, id DESC LIMIT 1) AS ano_escolar_actual,
               (SELECT fecha_inscripcion FROM inscripciones WHERE estudiante_id = e.id ORDER BY fecha_inscripcion DESC, id DESC LIMIT 1) AS ultima_fecha_inscripcion,
               (SELECT id FROM inscripciones WHERE estudiante_id = e.id ORDER BY fecha_inscripcion DESC, id DESC LIMIT 1) AS ultimo_id_inscripcion
        FROM estudiantes e
    ...
    // ===== ORDENAMIENTO POR FECHA DE INSCRIPCIÓN (más reciente/antiguo) =====
    switch ($orden) {
        case 'nombre_asc':
            $sql .= " ORDER BY e.nombre ASC, e.apellido ASC, e.sala";
            break;
        case 'nombre_desc':
            $sql .= " ORDER BY e.nombre DESC, e.apellido DESC, e.sala";
            break;
        case 'antiguo':
            // Más antiguo: fecha de inscripción más baja primero, NULL al final
            $sql .= " ORDER BY IF(ultima_fecha_inscripcion IS NULL, 1, 0), ultima_fecha_inscripcion ASC, ultimo_id_inscripcion ASC, e.sala, e.nombre, e.apellido";
            break;
        case 'reciente':
        default:
            // Más reciente: fecha de inscripción más alta primero, NULL al final
            $sql .= " ORDER BY IF(ultima_fecha_inscripcion IS NULL, 1, 0), ultima_fecha_inscripcion DESC, ultimo_id_inscripcion DESC, e.sala, e.nombre, e.apellido";
            break;
    }
    ...
    // Si hay error en la consulta, devolverlo en el JSON
    if (!$result) {
...
$busqueda = isset($_GET['busqueda']) ? trim($_GET['busqueda']) : '';
$filtro_anio = isset($_GET['anio']) ? trim($_GET['anio']) : '';
$orden_seleccionado = isset($_GET['orden']) ? trim($_GET['orden']) : 'reciente';
...
?>
<!-- ===== HTML Y JAVASCRIPT COMPLETO (con filtro de orden por fecha) ===== -->
<style>
    :root { 
    ...
    #busquedaInput, #ordenSelect {
        transition: all 0.3s ease;
    }
    #busquedaInput:focus, #ordenSelect:focus {
        border-color: #002d54;
        ...
        <div class="card-body p-4">
            <form method="GET" action="listado.php" id="filtroForm" autocomplete="off">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        <label class="small fw-bold text-muted"><i class="fas fa-sort me-1"></i> Ordenar por</label>
                        <select name="orden" id="ordenSelect" class="form-select shadow-none">
                            <option value="reciente" <?= ($orden_seleccionado == 'reciente') ? 'selected' : '' ?>>Más reciente</option>
                            <option value="antiguo" <?= ($orden_seleccionado == 'antiguo') ? 'selected' : '' ?>>Más antiguo</option>
                            <option value="nombre_asc" <?= ($orden_seleccionado == 'nombre_asc') ? 'selected' : '' ?>>A - Z</option>
                            <option value="nombre_desc" <?= ($orden_seleccionado == 'nombre_desc') ? 'selected' : '' ?>>Z - A</option>
                        </select>
                    </div>
                </div>
            </form>
        ...
        .catch((error) => {
            ...
            tablaContainer.innerHTML = `
                <div class="text-center py-5 text-danger">
                    ...
                    <br><small class="text-muted">Revise la consola para más detalles.</small>
                </div>
            `;
        });
}
...
ordenSelect.addEventListener('change', cargarTabla);
