...
if (empty($nombre) || empty($apellido) || empty($fecha_nac) || empty($genero) ||
    empty($rep_nombre) || empty($rep_cedula) || empty($rep_telefono) ||
    empty($cedula_base) || empty($ano_escolar_arr) || empty($grado_seccion_arr)) {
...
// ========== DETERMINAR SALA Y SECCIÓN DESDE EL ÚLTIMO GRADO ==========
...
try {
    ...
    if ($representante_existente_id > 0) {
        $stmt_rep = $conexion->prepare("UPDATE representantes SET 
            ...
            WHERE id = ?");
        if (!$stmt_rep) throw new Exception("Error SQL en update representante: " . $conexion->error);
        ...
        $stmt_rep = $conexion->prepare("INSERT INTO representantes 
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        if (!$stmt_rep) throw new Exception("Error SQL en insert representante: " . $conexion->error);
    ...
    if ($es_nuevo) {
        $stmt_est = $conexion->prepare("INSERT INTO estudiantes 
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Activo', 0, NOW())");
        if (!$stmt_est) throw new Exception("Error SQL en insert estudiante: " . $conexion->error);
        ...
        $stmt_est = $conexion->prepare("UPDATE estudiantes SET 
            ...
            orden_nacimiento = ? 
            WHERE id = ?");
        if (!$stmt_est) throw new Exception("Error SQL en update estudiante: " . $conexion->error);
    ...
    if ($estudiante_id > 0) {
        ...
        if (!$stmt_del_ins) throw new Exception("Error SQL en delete inscripciones: " . $conexion->error);
    ...
    $stmt_ins = $conexion->prepare("INSERT INTO inscripciones 
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '', ?, ?)");
    if (!$stmt_ins) throw new Exception("Error SQL en insert inscripciones: " . $conexion->error);
    ...
    if ($ingreso_id > 0) {
        ...
        if (!$stmt_del_ing) throw new Exception("Error SQL en delete ingresos: " . $conexion->error);
    ...
    $estado = verificarFichaCompleta($estudiante_id, $conexion);
    ...
    if (!$stmt_completa) throw new Exception("Error SQL en update inscripcion_completa: " . $conexion->error);
   ...
   // ========== REDIRIGIR SIEMPRE A INSCRIPCIÓN EXITOSA ==========
if ($inscripcion_completa) {
    header("Location: inscripcion_exito.php?id=$estudiante_id&completa=1");
} else {
    header("Location: inscripcion_exito.php?id=$estudiante_id&completa=0");
}
exit();
