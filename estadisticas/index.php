...
<style>
    ...
    .matricula-box input { width: 45px; border: 1px solid #ced4da; rounded: 4px; text-align: center; font-weight: bold; background: #ffffff; color: #000000; }
    .matricula-box input:focus { outline: none; border-color: var(--navy); }
</style>
        ...
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex gap-3 align-items-center">
                ...
                <div class="bg-white text-dark px-3 py-1 rounded small fw-bold matricula-box">
                    Matrícula: V <input type="number" min="0" name="mat_v" id="mat_v" value="<?= $m_v ?>" oninput="recalcular()"> 
                    H <input type="number" min="0" name="mat_h" id="mat_h" value="<?= $m_h ?>" oninput="recalcular()">
                </div>
            </div>
        </div>
                ...
                <tbody>
                    <tr>
                        ...
                        <?php for($d=1; $d<=$dias_en_mes; $d++): 
                            ...
                            <td class="<?= $es_fin ? 'weekend-cell' : 'p-0' ?>">
                                <?php if(!$es_fin): ?>
                                    <input type="number" min="0" name="asist_v[<?= $d ?>]" class="asist-input in-v" data-dia="<?= $d ?>" value="0" oninput="recalcular()">
                                <?php endif; ?>
                            </td>
                        <?php endfor; ?>
                    ...
                    <tr>
                        ...
                        <?php for($d=1; $d<=$dias_en_mes; $d++): 
                            ...
                            <td class="<?= $es_fin ? 'weekend-cell' : 'p-0' ?>">
                                <?php if(!$es_fin): ?>
                                    <input type="number" min="0" name="asist_h[<?= $d ?>]" class="asist-input in-h" data-dia="<?= $d ?>" value="0" oninput="recalcular()">
                                <?php endif; ?>
                            </td>
                        <?php endfor; ?>
...
window.recalcular = function() {
    ...
    const porcV = matVVal > 0 && diasHabilesVal > 0 ? Math.round((totalV / (diasHabilesVal * matVVal)) * 100) : 0;
    const porcH = matHVal > 0 && diasHabilesVal > 0 ? Math.round((totalH / (diasHabilesVal * matHVal)) * 100) : 0;
    const matTotal = matVVal + matHVal;
    const granTotal = totalV + totalH;
    const porcTotal = matTotal > 0 && diasHabilesVal > 0 ? Math.round((granTotal / (diasHabilesVal * matTotal)) * 100) : 0;
